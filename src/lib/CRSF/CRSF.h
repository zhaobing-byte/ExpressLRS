...
static inline uint16_t ICACHE_RAM_ATTR CalcCRC16(volatile uint8_t *data, int length, uint16_t crc)
{
    while (length--)
...
static inline uint8_t ICACHE_RAM_ATTR CalcCRC(volatile uint8_t *data, int length, uint8_t crc)
{
...
static inline uint8_t ICACHE_RAM_ATTR CalcCRC(uint8_t *data, int length, uint8_t crc)
{
    for (uint8_t i = 0; i < length; i++)
    {
        crc = crc8tab[crc ^ *data++];
